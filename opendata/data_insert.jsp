...
<%
	...
	if(!chkInp(test)) {
		%><script> alert('삽입에 실패하였습니다. 조건을 확인하여 주십시오.'); location.href='data_insert_main.jsp';</script><%
	} else { 
		for(int i = 0; i < 7; i++) {
			arr[i] = Integer.parseInt(test[i]);
		}
	
		pstmt = conn.prepareStatement("insert into data (nd, jh, ji, gh, gi, gc, cn) values(?, ?, ?, ?, ?, ?, ?)");
	
		pstmt.setInt(1, arr[0]);
		pstmt.setInt(2, arr[1]);
		pstmt.setInt(3, arr[2]);
		pstmt.setInt(4, arr[3]);
		pstmt.setInt(5, arr[4]);
		pstmt.setInt(6, arr[5]);
		pstmt.setInt(7, arr[6]);


		try {
			pstmt.executeUpdate();
			%><script> alert('삽입에 성공하였습니다.'); location.href='data_insert_main.jsp';</script><%
		}catch(Exception e){
			%><script> alert('삽입에 실패하였습니다. 조건을 확인하여 주십시오.'); location.href='data_insert_main.jsp';</script><%
		}
	}	
